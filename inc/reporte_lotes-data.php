	...
	switch ($q) {
        ...
        case 'ver':
            ...
            if (isset($_REQUEST['search']) && !empty($_REQUEST['search'])){
                ...
                $where = " AND CONCAT_WS(' ', lp.id_lote, lp.fecha_tope, c.razon_social, l.encargado, r.nombre) LIKE '%$search%'";
            }
            ...
            // DATE_FORMAT(lp.fecha_tope, '%d/%m/%Y') AS fecha_tope, c.razon_social AS entidad
            ...
            // ORDER BY $sort $order LIMIT $offset, $limit");
						...
						if ($id_clientex!='null') {
							$db->setQuery("SELECT SQL_CALC_FOUND_ROWS lp.id as id_pedido,lp.id_lote AS nro_lote, l.cantidad ,l.cantidad_actual,
							round((l.cantidad_actual*100)/l.cantidad) as porcentaje,
							l.encargado, r.nombre AS proveedor,
							DATE_FORMAT(l.fecha_tope, '%d/%m/%Y') AS fecha_tope, c.razon_social AS entidad
							FROM lista_pedidos lp
							LEFT JOIN lotes l ON l.id_lote=lp.id_lote
							LEFT JOIN repartidores r ON l.id_repartidor=r.id_repartidor
							LEFT JOIN clientes c ON l.id_entidad=c.id_cliente
        ...
        case 'buscar_usuario_final':
			...
			if ($rows){
				foreach ($rows as $r){
					$salida[] = ['ruc' => $r->ruc, 'razon_social' => $r->razon_social, 'total_count'=>$count];
				}
			}else{
				$salida[] = ['ruc' => '', 'razon_social' => '', 'total_count'=>''];
			}
