...
header('Access-Control-Allow-Methods: GET, POST');
	...
	switch ($q){
		...
		case 'ver':
			...
			$db->setQuery("SELECT  cp.id_cupos_proveedor, cp.id_cupo, cp.id_proveedor, cp.cantidad as cantidad_asignada, c.id_cliente, c.id_puerto,
c.fecha, c.cantidad as cantidad_disponible, cl.razon_social, p.puerto, t.id_ticket, t.id_chofer, t.chapa, t.fecha_de_creacion, ch.nombre as chofer, pr.nombre as proveedor
			FROM tickets t
      left join cupos_proveedor cp on t.id_cupos_proveedor=cp.id_cupos_proveedor
		...
		case 'cargar_ticket':
			...
			$db->setQuery("INSERT INTO tickets (id_cupos_proveedor, id_chofer, chapa, fecha_de_creacion)
							VALUES ('$hidden_id_cupos_proveedor','$chofer','$chapa','$fecha_chofer')");
			if(!$db->alter()){
				echo "Error. ".$db->getError();
			}else{
			...
			$db->setQuery("UPDATE cupos_proveedor SET estado='ASIGNADO'
		...
		case 'eliminar':
			...
			$db->setQuery("UPDATE cupos_proveedor SET estado='DISPONIBLE'
				WHERE id_cupos_proveedor = '$id_cupos_proveedor'");

			if(!$db->alter()){
				echo "Error. ". $db->getError();
			}else{
				echo "<BR>Estado cambiado a 'DISPONIBLE'";
			}
		...
		case 'ver_cupos':
			...
			$db->setQuery("SELECT cp.id_cupos_proveedor, cp.id_cupo, cp.id_proveedor,c.id_cliente,c.id_puerto, cp.cantidad cantidad_disponible, cp.estado, c.fecha as fecha_cupo, cl.razon_social, p.puerto
